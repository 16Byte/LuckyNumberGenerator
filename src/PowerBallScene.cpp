...
void PowerBallScene::Draw()
{
    ...
    // Title
    DrawText("PowerBall Number Generator", 250, 50, 40, WHITE);
    ...
    if (GuiButton((Rectangle){buttonX, buttonY + 70, buttonWidth, buttonHeight}, "Generate PowerBall Numbers"))
    {
    ...
    if (!luckyNumbers.empty())
    {
        ...
        // Display the powerball
        if (luckyNumbers.size() > 5)
        {
            float powerBallX = startX + (5 * spacing) + 40;
            
            // Draw circle background
            DrawCircle(powerBallX, numberY, circleSize / 2, RED);
            
            // Draw number
            string numText = to_string(luckyNumbers[5]);
            int textWidth = MeasureText(numText.c_str(), 30);
            DrawText(numText.c_str(), powerBallX - textWidth / 2, numberY - 15, 30, WHITE);
            
            // Draw quality
            int quality = (int)generator->GetNumberQualityAtPosition(luckyNumbers[5], 5);
            string qualityText = to_string(quality) + "%";
            int qWidth = MeasureText(qualityText.c_str(), 16);
            DrawText(qualityText.c_str(), powerBallX - qWidth / 2, numberY + 40, 16, WHITE);
            
            // Draw button (toggle lock)
            string buttonText = (lockedNumbers[5]) ? "L" : "^";
            if (GuiButton((Rectangle){powerBallX - buttonSize / 2, numberY + 65, buttonSize, buttonSize}, buttonText.c_str()))
            {
